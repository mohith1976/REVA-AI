...
export default function ProfilePage() {
  ...
  const inputCls = `w-full px-4 py-3 border rounded-xl text-sm transition-all ${
    isEditing
  ...
  const labelCls =
    "flex items-center gap-2 text-[10px] font-bold text-neutral-400 uppercase tracking-[1.5px] mb-2 px-1";
      ...
      <nav className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-neutral-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-8 flex items-center justify-between h-[64px]">
          ...
          >
            ...
            <span className="font-bold text-[17px] tracking-tight text-neutral-900">
              REVA AI
            </span>
          </Link>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            <Link
              href="/track"
              className="px-3 py-1.5 text-[13px] font-semibold text-neutral-500 hover:text-neutral-900 transition-colors rounded-lg hover:bg-neutral-50"
            >
              {t("nav.track")}
            </Link>
            {user && (
      ...
      <div className="max-w-xl mx-auto py-12 sm:py-14 px-4 sm:px-6">
        ...
        <div className="bg-white border border-neutral-100 rounded-[28px] p-6 sm:p-10 shadow-2xl shadow-black/5 mt-10">
          ...
          <div className="mb-8 flex items-center justify-between">
            <div>
              <p className="text-[10px] sm:text-[0.72rem] font-bold tracking-[2px] uppercase text-neutral-400 mb-1">
                {t("profile.account")}
              </p>
          ...
          <form className="space-y-6" onSubmit={handleUpdate}>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  type="text"
                  className={`${inputCls} flex-1 font-mono tracking-tight ${gettingLocation ? "animate-pulse ring-2 ring-neutral-900/10" : ""}`}
                  value={
                    formData.latitude
                  ...
                  readOnly
                  placeholder={
                    gettingLocation ? "Locating..." : t("profile.noLocation")
                  }
                />
                {isEditing && (
                  <button
                    ...
                    className="px-4 py-2 bg-neutral-900 text-white text-[13px] font-bold rounded-xl hover:bg-neutral-800 transition-all shadow-sm flex items-center gap-2 min-w-[110px] justify-center disabled:opacity-70"
                  >
                    {gettingLocation ? (
                    ...
                    <span>
                      {gettingLocation ? "Locating..." : t("profile.getGps")}
                    </span>
                  </button>
                )}
              </div>
            ...
            {user?.policeStation && (
              <div className="p-5 bg-neutral-50 border border-neutral-100 rounded-[20px]">
                <div className="text-[10px] font-bold uppercase text-neutral-400 tracking-[1.5px] mb-2 px-1">
                ...
                <div className="text-[13px] text-neutral-500">
                  {user.policeStation.district}, {user.policeStation.state}
                </div>
              </div>
            )}
